...
SELECT * FROM EMP; -- 사번, 이름, 업무(직책), 상사사번, 입사일, 급여, 상여, 부서번호
...
SELECT *                                         -- (3)
    FROM EMP E, DEPT D                           -- (1) 
    WHERE ENAME='SCOTT' AND E.DEPTNO=D.DEPTNO;   -- (2) FROM이 먼저 실행하기에 TABALE에 별칭을 지정했다면 별칭으로 사용해야한다
...
SELECT ENAME, E.DEPTNO, LOC FROM EMP E, DEPT D
    ...
    -- EX3. 근무지가 CHICAGO인 사람의 이름, 급여, 부서번호를 출력
SELECT ENAME, SAL, E.DEPTNO FROM EMP E, DEPT D
    WHERE E.DEPTNO=D.DEPTNO AND LOC='CHICAGO';
    -- EX4. JOB이 'SALESMAN'이거나 'MANAGER'인 사원의 이름, 급여, 상여, 연봉, 부서명
        --연봉이 큰 순서로 정렬, 단 연봉 = (SAL+COMM)*12
SELECT ENAME 이름, COMM 상여, (SAL+NVL(COMM,0))*12 연봉, DNAME 부서명
    FROM EMP E, DEPT D
    WHERE E.DEPTNO=D.DEPTNO AND JOB IN ('SALESMAN', 'MANAGER')
    ORDER BY 연봉 DESC;
    -- EX5. COMM이 NULL이고, SAL 1200이상인 사원의 이름, 급여, 부서번호, 부서명
        -- 부서명순, 급여 큰순 정렬
SELECT ENAME 이름, SAL 급여, E.DEPTNO 부서번호, DNAME 부서명
    FROM EMP E, DEPT D
...
SELECT * FROM EMP, SALGRADE WHERE ENAME='SCOTT' AND SAL BETWEEN LOSAL AND HISAL;
    --EX1. 모든 사원의 사번, 이름, 급여, 급여등급, 상사사번
SELECT DEPTNO, ENAME, SAL, GRADE||'등급' GRADE, MGR FROM EMP, SALGRADE
    WHERE SAL BETWEEN LOSAL AND HISAL;
    --탄탄1. Comm이 null이 아닌 사원의 이름, 급여, 등급, 부서번호, 부서이름, 근무지를 출력하시오
SELECT ENAME, SAL, GRADE||'등급' GRADE, E.DEPTNO, DNAME, LOC
...
SELECT ENAME, SAL, GRADE, (SAL+NVL(COMM,0))*12 ANNSAL, DNAME
    FROM EMP E, DEPT D, SALGRADE
    WHERE E.DEPTNO=D.DEPTNO AND SAL BETWEEN LOSAL AND HISAL
    ORDER BY DNAME, ANNSAL;
...
SELECT ENAME, JOB, SAL, GRADE, E.DEPTNO, DNAME FROM EMP E, DEPT D, SALGRADE
    ...
    --틴탄5.	이름, 급여, 등급, 입사일, 근무지. 81년에 입사한 사람. 등급 큰순
...
SELECT EMPNO, ENAME, JOB, SAL, GRADE FROM EMP E, DEPT D, SALGRADE
    WHERE E.DEPTNO=D.DEPTNO AND SAL BETWEEN LOSAL AND HISAL
    ORDER BY SAL DESC;
...
SELECT WORKER.EMPNO 사번, WORKER.ENAME 이름, WORKER.MGR, MANAGER.EMPNO 상사사번, MANAGER.ENAME 상사이름 
    FROM EMP WORKER, EMP MANAGER
    WHERE WORKER.MGR = MANAGER.EMPNO;
    -- EX1. 상사가 있는 사원의 사번, 이름, 상사사번, 상사이름
SELECT W.EMPNO, W.ENAME, W.MGR, M.ENAME
    FROM EMP W, EMP M
    WHERE W.MGR=M.EMPNO;
    -- EX2. 'SMITH의 상사는 FORD다' 포맷으로 출력
SELECT W.ENAME||'의 상사는 '||M.ENAME||'다' MSG
    FROM EMP W, EMP M
    WHERE W.MGR=M.EMPNO;
...
-- 4. OUTER JOIN : JOIN시 조건에 만족하지 않는 행까지 나타나게
...
SELECT W.ENAME, W.SAL, GRADE, DNAME, (W.SAL+NVL(W.COMM,0))*12 ANNSAL, M.ENAME
    FROM EMP W, EMP M, SALGRADE, DEPT D
    WHERE W.MGR=M.EMPNO(+) AND W.DEPTNO=D.DEPTNO AND W.SAL BETWEEN LOSAL AND HISAL;

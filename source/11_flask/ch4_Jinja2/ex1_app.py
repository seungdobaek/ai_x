...
# 2. 제어문 {% %}
    # 2-1 조건문 {% if 조건 %}A태그{% elif 조건 2 %}B태그{% else %}C태그(% endif %)
    # 2-2 반복문
        # {% for var in vars %}
            # <태그>{{loop.index}}. {{var}} </태그>
                # loop.index : 1부터 순번 / loop.first:첫번째 loop인지 여부 / loop.last : 마지막 loop인지 여부
        # {% endfor %}
# 3. 헤더나 풋터 {% include "header.html" %} {% extends "base.html" %}
# 4. 서브 블록 {% block %} {endblock}
# 5. 주석 (# 주석 #)
...
def index():
    ...
    cnt = len(lst) # lst 개수
    price = 12000
    return render_template('1_index.html', name=name, cnt=cnt, names=lst, price=price)
